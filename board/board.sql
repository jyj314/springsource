...
-- 타이틀 모달
-- pageNum=1&amount=10&type=T&keyword=배고프다

select bno, title, writer, regdate, updatedate
from (select /*+INDEX_DESC(spring_board pk_spring_board)*/rownum rn, bno, title, writer, regdate, updatedate
	from spring_board
	where bno > 0 and (title like '%배고프다%') and rownum <= (1 * 30))
where rn > (1-1) * 30;

-- pageNum=1&amount=10&type=TC&keyword=배고프다

select bno, title, writer, regdate, updatedate
from (select /*+INDEX_DESC(spring_board pk_spring_board)*/rownum rn, bno, title, writer, regdate, updatedate
	from spring_board
	where bno > 0 and (title like '%배고프다%' or content like '%배고프다%') and rownum <= (1 * 30))
where rn > (1-1) * 30;
...
select * from spring_reply where bno=788;

select * from spring_board where bno=788;
